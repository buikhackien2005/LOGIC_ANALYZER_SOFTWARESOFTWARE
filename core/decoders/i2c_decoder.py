...
class I2CDecoder(ProtocolDecoder):
    def __init__(self, sda_channel, scl_channel):
        ...
        self.sda_channel = int(str(sda_channel).replace('CH', '').strip())
        self.scl_channel = int(str(scl_channel).replace('CH', '').strip())
    ...
    def decode(self, sample_rate, buffer_array):
        ...
        sda_diff = np.diff(sda.astype(np.int8))
        ...
        for start_idx in start_conditions:
            ...
            transactions.append({
                'time_val': time_sec,  # ĐÃ BỔ SUNG KHÓA ĐỒNG BỘ
